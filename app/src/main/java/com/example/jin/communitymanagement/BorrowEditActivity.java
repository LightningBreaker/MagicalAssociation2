...
public class BorrowEditActivity extends AppCompatActivity {
    ...
    private void initView() {
        picture=(ImageView)findViewById(R.id.imageView_edit_borrow_img);
        dialogBuilder=NiftyDialogBuilder.getInstance(this);
        edit_introduction=(EditText)findViewById(R.id.editView_edit_borrow_introduction);
        edit_asso_name=(EditText)findViewById(R.id.editView_edit_borrow_name);
        edit_connection=(EditText)findViewById(R.id.EditView_edit_connection_name);
        fab_ok=(FloatingActionButton)findViewById(R.id.fab_edit_borrow_ok);
    ...
    private void add_new_activity() {
        ...
        String borrow_name=edit_asso_name.getText().toString();
        ...
        {
            Toast.makeText(this, "请选择租赁时间", Toast.LENGTH_SHORT).show();
        }
        else
        {
            if(borrow_name.length()<2)
            {
                Toast.makeText(this, "请完善租赁物品名字", Toast.LENGTH_SHORT).show();
            }
            else
            {
                if(introduction.length()<10)
                {
                    Toast.makeText(this, "租赁物品介绍需要在10字以上", Toast.LENGTH_SHORT).show();
                }
                else
                {
                    if(m_photo==null)
                    {
                        Toast.makeText(this, "请选择物品照片", Toast.LENGTH_SHORT).show();
                    }
                    else
                    {
                        add_activity_to_database(m_start,m_end,borrow_name,introduction,m_photo,connection);
                    }
                }
            }
        }
    ...
    private void add_activity_to_database(String m_start, String m_end, String borrow_name, String introduction, Bitmap m_photo,String connection) {
        byte[] bytes=get_bit_image(m_photo);
        SQLiteDatabase db=dbHelper.getWritableDatabase();
        db.execSQL("insert into BorrowTable(time_start,time_end,borrow_name,connection,introduction,image) values (?,?,?,?,?,?)"
                , new Object[] {m_start,m_end,borrow_name,connection,introduction,bytes});
    ...
    private void initAddImageView() {
        cardViewAddImage=(CardView)findViewById(R.id.cardView_edit_borrow_add_photo);
    ...
    private void pickPictureFromFileCamera()
    {
        ...
        {
            imageUri= FileProvider.getUriForFile(BorrowEditActivity.this,"com.example.jin.communitymanagement.fileprovider",outputImage);
    ...
    private void initEditTimeCardView() {
        ...
        cardViewEditStartTime=(CardView)findViewById(R.id.cardView_edit_borrow_start_time);
        cardViewEditEndTime=(CardView)findViewById(R.id.cardView_edit_borrow_end_time);
        textViewEditStartTime=(TextView)findViewById(R.id.text_edit_borrow_start_time);
        textViewEditEndTime=(TextView)findViewById(R.id.text_edit_borrow_end_time);
        textEditTimeSV=(TextView)findViewById(R.id.text_edit_borrow_start_time_v);
        textEditTimeEV=(TextView)findViewById(R.id.text_edit_borrow_end_time_v);
